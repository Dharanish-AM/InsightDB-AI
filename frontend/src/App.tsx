...
import { QueryHistoryView } from './components/QueryHistory';

export function App() {
  const [user, setUser] = useState<User | null>(null);
  const [loadingUser, setLoadingUser] = useState(true);
  const [activeTab, setActiveTab] = useState<'studio' | 'connections' | 'schema' | 'history'>('studio');
  ...
  useEffect(() => {
    (async () => {
      try {
        if (localStorage.getItem('insightdb_token')) {
  ...
  const handleSelectHistoryQuery = (_sql: string, queryText: string) => {
    setSelectedHistoryQuery(queryText);
    setActiveTab('studio');
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="flex items-center gap-3 text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>
          <span className="w-8 h-8 rounded-xl animate-pulse bg-violet-600" />
  ...
  if (!user) {
    return <AuthView onSuccess={async u => { setUser(u); await fetchConnections(); }} />;
  }
